...
export const TargetTile = freeze({
	...
	translate(idx, {arrived,trPos,isInHouse,isScatter,isFright,isToHouse}, {x, y}) {
		const isFrozen = (!isToHouse && Timer.frozen)
		if (isToHouse)
		...
		if (idx == GhsEnum.Guzuta) {
			...
			Circle.opacity(isFrozen || hidden ? 0:1).transform(T*x-r, T*y-r)
		}
		if (idx == GhsEnum.Aosuke && !arrived)
			return
		Tiles[idx]
			.opacity(isFrozen || isFright || isInHouse ? 0:.8)
			.transform(T*x, T*y)
	}
